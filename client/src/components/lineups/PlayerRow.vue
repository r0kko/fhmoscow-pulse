    ...
    <td v-if="isDouble">
        ...
        :value="
          (function () {
            if (isPlayerGk(p) && gkCount === 3) {
              return (editedBoth[p.team_player_id] ?? p.squad_both ?? false)
                ? 'both'
                : String(editedSquad[p.team_player_id] ?? p.squad_no ?? '');
            }
            return String(editedSquad[p.team_player_id] ?? p.squad_no ?? '');
          })()
        "
          ...
          'is-invalid':
            ...
            !(function () {
              if (
                ...
                return true;
              const v = editedSquad[p.team_player_id] ?? p.squad_no ?? null;
              return v === 1 || v === 2;
            })(),
        }"
      ...
      <div
        v-if="
          ...
          !(
            (editedSquad[p.team_player_id] ?? p.squad_no ?? null) === 1 ||
            (editedSquad[p.team_player_id] ?? p.squad_no ?? null) === 2
          )
        "
        class="invalid-feedback d-block"
      >
