...
async function save() {
  ...
  try {
    if (editing.value) {
      await apiFetch(`/normative-groups/${editing.value.id}`, { method: 'PUT', body: JSON.stringify(payload) });
    } else {
      await apiFetch('/normative-groups', { method: 'POST', body: JSON.stringify(payload) });
    }
    modal.hide();
    await load();
  } catch (e) {
          ...
          <table class="table table-striped align-middle mb-0">
            ...
            <tbody>
              <tr v-for="g in groups" :key="g.id">
                <td>{{ seasons.find(s => s.id === g.season_id)?.name || g.season_id }}</td>
                <td>{{ g.name }}</td>
                <td class="text-end">
