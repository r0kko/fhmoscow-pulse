...
test('matchController falls back to local when external is unavailable', async () => {
  ...
  expect(json).toHaveBeenCalledWith(
    expect.objectContaining({ external_available: false, page: 1, total_pages: 1 })
  );
});
...
test('matchController uses external when available', async () => {
  ...
  const req = { query: { page: '2', limit: '5', type: 'away', q: 'x' }, user: { id: 'u2' } };
  const json = jest.fn();
  const res = { json };
  const next = jest.fn();
...
test('matchController forces local when source=local even if external is available', async () => {
  jest.resetModules();
  const listUpcomingLocalMock2 = jest.fn().mockResolvedValue({ rows: [{ id: 'x' }], count: 1 });
  jest.unstable_mockModule('../src/services/matchService.js', () => ({
    __esModule: true,
  ...
  const { default: controllerX } = await import('../src/controllers/matchController.js');
  ...
  expect(json).toHaveBeenCalledWith(expect.objectContaining({ external_available: false }));
});
...
test('matchController forwards errors via next()', async () => {
  ...
  jest.unstable_mockModule('../src/services/matchService.js', () => ({
    __esModule: true,
    default: { listUpcoming: async () => { throw err; } },
    listUpcomingLocal: async () => { throw err; },
  }));
  jest.unstable_mockModule('../src/config/externalMariaDb.js', () => ({
    __esModule: true,
    isExternalDbAvailable: () => true,
...
test('matchController defaults to type=all on invalid type', async () => {
  ...
  const { default: controller5 } = await import('../src/controllers/matchController.js');
