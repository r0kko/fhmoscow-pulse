...
const { default: controller } = await import('../src/controllers/matchController.js');
...
test('listPast falls back to local when external is unavailable', async () => {
  ...
  const req = { query: { page: '1', limit: '10', type: 'home', q: 'x', season_id: 's1' }, user: { id: 'u' } };
  const res = { json: jest.fn() };
  await controller.listPast(req, res, () => {});
  expect(listPastLocalMock).toHaveBeenCalled();
  expect(res.json).toHaveBeenCalledWith(
    expect.objectContaining({ external_available: false, total_pages: 1, page: 1 })
  );
});
...
test('listPast uses external when available', async () => {
  ...
  const { default: controller2 } = await import('../src/controllers/matchController.js');
  const req = { query: { page: '2', limit: '5', type: 'away', q: 'k' }, user: { id: 'u' } };
  const res = { json: jest.fn() };
  await controller2.listPast(req, res, () => {});
  expect(listPastMock2).toHaveBeenCalled();
  expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ external_available: true, page: 2 }));
});

test('listPast forces local when source=local', async () => {
  jest.resetModules();
  const listPastLocalMock2 = jest.fn().mockResolvedValue({ rows: [{ id: 'x' }], count: 1 });
  jest.unstable_mockModule('../src/services/matchService.js', () => ({
    __esModule: true,
  ...
  const { default: controller3 } = await import('../src/controllers/matchController.js');
  ...
  expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ total_pages: 1, external_available: false }));
});
...
test('listPast forwards errors via next()', async () => {
  ...
  jest.unstable_mockModule('../src/services/matchService.js', () => ({
    __esModule: true,
    default: { listPast: async () => { throw err; }, listPastLocal: async () => { throw err; } },
    listUpcomingLocal: jest.fn(),
  }));
  jest.unstable_mockModule('../src/config/externalMariaDb.js', () => ({
    __esModule: true,
  ...
  const { default: controller4 } = await import('../src/controllers/matchController.js');
  const next = jest.fn();
  await controller4.listPast({ query: {}, user: { id: 'u' } }, { json: jest.fn() }, next);
