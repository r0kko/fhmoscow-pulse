...
export default function createTrainingAdminController(forCamp) {
  return {
    async list(req, res) {
      const {
        page = '1',
    ...
    async get(req, res) {
      try {
        const training = await trainingService.getById(req.params.id, forCamp);
    ...
    async remove(req, res) {
      try {
        await trainingService.remove(req.params.id, req.user.id, forCamp);
