...
async function buildElectronicInteractionAgreementPdf(user, meta = {}) {
    ...
    const measure = (doc, fonts, token, bold) => {
        const prev = doc._font;
        try { doc.font(bold ? fonts.bold : fonts.regular); } catch { /* noop */ }
        const w = doc.widthOfString(token);
        if (prev) { try { doc.font(prev.name || prev); } catch { /* noop */ } }
        return w;
    };
    const layoutJustifiedStyledParagraph = (doc, fonts, segments, width) => {
        const startX = doc.page.margins.left;
        ...
        while (i < styledTokens.length) {
            const { text, bold } = styledTokens[i];
            const isWs = /^\s+$/.test(text);
            if (isWs && (!line.length || /^\s+$/.test(line[line.length - 1]?.text))) { i++; continue; }
            const tokenWidth = measure(doc, fonts, text, bold);
            const maxWidth = width;
            const remaining = maxWidth - lineWidth;
            if (tokenWidth <= remaining || (!line.length && tokenWidth <= maxWidth)) {
        ...
        lines.forEach((ln, idx) => {
            ...
            const extraPerSpace = stretchPoints > 0 ? extra / stretchPoints : 0;
            ln.items.forEach((it) => {
                try { doc.font(it.bold ? fonts.bold : fonts.regular); } catch { /* noop */ }
                doc.text(it.text, x, y, { lineBreak: false });
                x += it.width + (isStretchableSpace(it.text) && !last ? extraPerSpace : 0);
            });
            y += lineHeight;
    ...
    const heading = 'Соглашение об электронном взаимодействии\nс использованием простой электронной подписи';
    try { doc.font(fonts.bold); } catch { /* empty */ }
    doc.fontSize(14).text(heading, { align: 'center' });
    try {
        doc.moveDown(0.2);
        doc.fillColor('#666666');
        const savedFont = doc._font ? doc._font.name : null;
        try { doc.font('SB-Italic'); } catch { /* empty */ }
        doc.fontSize(10).text('для спортивного судьи Федерации хоккея Москвы', { align: 'center' });
        if (savedFont) doc.font(savedFont);
        doc.fillColor('black');
    } catch { /* empty */ }
    try { doc.font(fonts.regular); } catch { /* empty */ }

    // Location/date line (single row: city left, date right)
    const docDate = meta.documentDate ? formatDateHuman(meta.documentDate) : '«__» __________ ____ г.';
    doc.moveDown(0.8);
    doc.fontSize(11);
    {
        const y = doc.y;
    ...
    // Fetch user details
    const [passports, userAddresses, innRec, snilsRec, userDetails] = await Promise.all([
        (Models.Passport?.findAll?.bind(Models.Passport) || (() => Promise.resolve([])))({
            where: { user_id: user.id },
            order: [['created_at', 'DESC']],
            attributes: ['series', 'number', 'issue_date', 'issuing_authority', 'issuing_authority_code', 'place_of_birth'],
        }),
        (Models.UserAddress?.findAll?.bind(Models.UserAddress) || (() => Promise.resolve([])))(
            {
                where: { user_id: user.id },
        ...
        (Models.Inn?.findOne?.bind(Models.Inn) || (() => Promise.resolve(null)))({ where: { user_id: user.id }, attributes: ['number'] }),
        (Models.Snils?.findOne?.bind(Models.Snils) || (() => Promise.resolve(null)))({ where: { user_id: user.id }, attributes: ['number'] }),
        (Models.User?.findByPk?.bind(Models.User) || (() => Promise.resolve(null)))(user.id, { attributes: ['email', 'phone'] }),
    ]);
    ...
    layoutJustifiedStyledParagraph(
        ...
        fonts,
        [
            seg('Региональная общественная организация «Федерация хоккея Москвы» (ФХМ), именуемая в дальнейшем «Федерация», в лице президента Николишина Андрея Васильевича, действующего на основании Устава, и '),
            seg(fio(user) || '____________________', true),
            ...
            seg(', СНИЛС '),
            seg(snilsNumber, true),
            seg(' — физическое лицо, планирующее заключить с Федерацией договор об осуществлении судейской деятельности (именуемому далее «Судья»), заключили настоящее Соглашения в порядке статьи 428 Гражданского кодекса РФ на нижеследующих условиях.'),
        ],
        contentWidth
    );
    ...
    doc.moveDown(0.3);
    doc.font(fonts.regular);
    doc.text('1.1. Судья – физическое лицо (самозанятое лицо, применяющее специальный налоговый режим «Налог на профессиональный доход», либо индивидуальный предприниматель), планирующее заключить или заключившее договор с Федерацией на осуществление обязанностей спортивного судьи (арбитра) по хоккею.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.2. Личный кабинет Судьи – защищённый раздел информационной системы Федерации (включая веб-платформу lk.fhmoscow.com), доступный Судье после авторизации и предназначенный для получения и подписания документов, обмена информацией, получения назначений на спортивные мероприятия и подтверждения таких назначений. Доступ к Личному кабинету осуществляется по уникальным учетным данным (логину и паролю) Судьи; действия, совершённые в Личном кабинете после входа под учетными данными Судьи, признаются совершенными лично Судьёй.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.3. Электронный документ – документированная информация, представленная в электронной форме, пригодной для восприятия человеком с помощью компьютеров, передаваемая и хранимая в информационно-телекоммуникационных сетях. Электронный документ в рамках настоящего Соглашения может быть создан, подписан и передан сторонами с использованием информационной системы Федерации или иных согласованных средств электронного взаимодействия.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.4. Электронная подпись (ЭП) – информация в электронной форме, присоединенная к Электронному документу либо иным образом связанная с ним, и используемая для определения лица, подписавшего этот документ. Виды электронных подписей, используемые Сторонами, определяются законодательством Российской Федерации и настоящим Соглашением.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.5. Простая электронная подпись (ПЭП) – вид Электронной подписи, который посредством использования кодов, паролей или иных средств подтверждает факт формирования подписи определенным лицом. В рамках настоящего Соглашения в качестве ПЭП могут использоваться, в том числе:', { align: 'justify' });
    doc.text(' • электронная подпись, формируемая средствами Личного кабинета (например, ввод Судьёй одноразового проверочного кода, направленного на подтвержденный номер телефона или адрес электронной почты Судьи, после ввода логина и пароля);');
    doc.text(' • электронная подпись, формируемая во внешнем сервисе электронного документооборота (например, сервис «Контур.Сайн»), включая усиленную неквалифицированную электронную подпись, признаваемую сторонами эквивалентом простой электронной подписи;');
    doc.text(' • факсимильное воспроизведение собственноручной подписи Судьи (скан-образ или фото подписи на бумажном документе), переданное посредством информационной системы Федерации или электронной почты. Стороны признают такой способ подписания допустимым аналогом собственноручной подписи в соответствии с частью 2 статьи 160 ГК РФ.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.6. Ключ простой электронной подписи – уникальный набор данных, известный Судье и используемый для создания и проверки ПЭП. В качестве Ключа ПЭП в системе Федерации используются персонализированные учетные данные Судьи (логин и пароль для доступа в Личный кабинет) и/или иные средства аутентификации, предоставленные Федерацией (например, одноразовый код подтверждения).', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('1.7. Стороны – Федерация и Судья, совместно упоминаемые в тексте Соглашения.', { align: 'justify' });
    ...
    doc.moveDown(0.3);
    doc.font(fonts.regular);
    doc.text('6.1. Настоящее Соглашение при акцепте Судьёй порождает те же правовые последствия, что и двустороннее письменное подписание на бумажном носителе. Стороны признают допустимость использования аналога собственноручной подписи и электронных подписей в соответствии с действующим законодательством РФ.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('6.2. Все споры и разногласия, связанные с исполнением настоящего Соглашения, разрешаются путем переговоров между Сторонами. При недостижении соглашения спор подлежит рассмотрению в судебном порядке по подсудности, установленной законодательством Российской Федерации. Заявления, претензии и иная официальная переписка между Сторонами в рамках исполнения Соглашения могут направляться в электронной форме (через Личный кабинет или на официальные электронные адреса Сторон) и будут признаваться действительными.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('6.3. В случае изменения законодательства или нормативных актов, затрагивающих отношения по настоящему Соглашению, Стороны будут руководствоваться действующим законодательством. Если какое-либо из положений Соглашения станет недействительным или невозможным к исполнению в силу изменений законодательства, это не затрагивает действительность остальных положений; Стороны при необходимости оформят соответствующее письменное изменение к Соглашению.', { align: 'justify' });
    doc.moveDown(0.3);
    doc.text('6.4. Настоящее Соглашение и отношения Сторон, не урегулированные им, подчиняются законодательству Российской Федерации. Стороны признают юридическую силу настоящего Соглашения в электронной форме. Оригинал текста Соглашения хранится в информационной системе Федерации. Судья в любой момент может скачать или запросить у Федерации экземпляр Соглашения на бумажном носителе, подписанный уполномоченным представителем Федерации.', { align: 'justify' });
    ...
    const drawRequisites = (x, title, groups) => {
        ...
        const drawSegments = (sx, sy, width, segments) => {
            ...
            const measure = (token, bold) => {
                const prev = doc._font;
                try { doc.font(bold ? fonts.bold : fonts.regular); } catch { /* empty */ }
                const w = doc.widthOfString(token);
                if (prev) { try { doc.font(prev.name || prev); } catch { /* empty */ } }
                return w;
            };
            const lineHeight = doc.currentLineHeight(true);
            let x = sx;
            ...
            (segments || []).forEach((seg) => {
                ...
                tokenize(seg.text).forEach((t) => tokens.push({ text: t, bold: !!seg.bold }));
            });
            let i = 0;
        ...
        const mk = (label, value) => ([{ text: `${label}: ` }, { text: String(value ?? ''), bold: true }]);
    ...
    const userPhoneMasked = userDetails?.phone ? maskRuPhone(userDetails.phone) : '____________________';
    const leftItems = [
        // 1. Name
        ...
        // 6. Address
        [
            [{ text: 'Адрес: ' }, { text: '101000, г. Москва, Кривоколенный пер., д. 9, стр. 1', bold: true }],
        ],
    ];
    const rightItems = [
        // 1. FIO
        [
            [{ text: 'ФИО: ' }, { text: fio(user) || '____________________', bold: true }],
        ],
        // 2. Email
        [
            [{ text: 'E-mail: ' }, { text: userDetails?.email ? clean(userDetails.email) : '____________________', bold: true }],
        ],
        // 3. Phone
        [
        ...
        // 5. Address
        [
            [{ text: 'Адрес регистрации: ' }, { text: `${regPostal}${regAddress}`, bold: true }],
        ],
        // 6. INN
        [
    ...
    const sigLineAt = (x, y, role, nameHint) => {
        ...
        try {
            doc.save().lineWidth(0.7).moveTo(x, lineY).lineTo(x + lineWidth, lineY).stroke().restore();
        } catch { /* empty */ }
        const hint = nameHint || '';
    ...
    for (let i = 0; i < range.count; i += 1) {
        doc.switchToPage(range.start + i);
        await applyFooter(doc, { page: i + 1, total: range.count, barcodeText, numberText });
    }
