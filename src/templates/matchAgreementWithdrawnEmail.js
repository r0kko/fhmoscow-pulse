...
export function renderMatchAgreementWithdrawnEmail(event) {
  ...
  const text =
    `${greet}\n\n` +
    'Ранее отправленная заявка на согласование матча была отозвана.\n' +
    ...
    `Дата и время: ${when}\n` +
    `Стадион: ${ground}\n\n` +
    `Открыть карточку согласования: ${appUrl}/school-matches/${event.matchId}/agreements`;

  const html = `
    <div style="font-family: Arial, sans-serif; color: #333;">
      <p style="font-size:16px;margin:0 0 16px;">${greet}</p>
      <p style="font-size:16px;margin:0 0 16px;">Ранее отправленная заявка на согласование матча была <strong>отозвана</strong>.</p>
