...
export function renderMatchAgreementProposedEmail(event) {
  ...
  const subject = `Новая заявка на согласование матча: ${event.team1} — ${event.team2}`;

  const when = formatMsk(event.kickoff);
  const ground = event.ground ? event.ground : '—';
  const greet = 'Здравствуйте!';
  const proposer =
    event.by === 'home' ? 'домашней командой' : 'гостевой командой';

  const text =
    `${greet}\n\n` +
    `Поступила новая заявка на согласование матча (${proposer}).\n` +
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; color: #333;">
      ...
      <p style="font-size:16px;margin:0 0 16px;">Поступила новая заявка на согласование матча (<strong>${escapeHtml(
        proposer
      )}</strong>).</p>
