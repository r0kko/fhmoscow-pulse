...
export function renderMatchAgreementApprovedEmail(event) {
  ...
  const text =
    `${greet}\n\n` +
    'Место и время матча согласованы.\n' +
    ...
    `Дата и время: ${when}\n` +
    `Стадион: ${ground}\n\n` +
    `Открыть карточку матча: ${appUrl}/school-matches/${event.matchId}/agreements`;

  const html = `
    <div style="font-family: Arial, sans-serif; color: #333;">
      <p style="font-size:16px;margin:0 0 16px;">${greet}</p>
      <p style="font-size:16px;margin:0 0 16px;"><strong>Место и время матча согласованы.</strong></p>
      ...
      <p style="margin:16px 0;">
        <a href="${appUrl}/school-matches/${event.matchId}/agreements" target="_blank" style="display:inline-block;background:#0F62FE;color:#fff;text-decoration:none;padding:10px 14px;border-radius:8px;font-weight:600;">Открыть карточку матча</a>
      </p>
    </div>`;
